...
from BadStyle.services.pyrogram import pbot as LYCIA
...
async def Lycia(_, message):
    if len(message.command) < 2:
        await message.reply_text("BadStyle AI Voice Chatbot")
        return
    text = message.text.split(None, 1)[1]
    lycia = text.replace(" ", "%20")
    m = await message.reply_text("BadStyle Is Best...")
    try:
        L = await fetch(
            f"https://api.affiliateplus.xyz/api/chatbot?message={lycia}&botname=BadStyle&ownername=Bad_Style&user=1"
        )
        chatbot = L["message"]
        ...
        await m.edit(str(e))
        return
    await m.edit("Made By @bad_style...")
